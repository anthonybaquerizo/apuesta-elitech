...
div.row.justify-content-center
    div.col-lg-6.col-md-6.col-sm-12.col-12
        div.card
            div.card-header
                h3 Perfil del usuario ({{ $store.getters['user/isAdmin'] ? 'Administrador' : 'Participante' }})
            div.card-body
                div.row.form-group
...
export default {
    ...
    created () {
        this.$store.commit('user/clearPassword')
    },
    methods: {
        save () {
            this.loading = true
            axios.put('api/user/profile', {
                ...
                .then(({ data }) => {
                    this.$store.commit('addAlert', {
                        ...
                        messages: [['Su usario fue actualizado correctamente!']]
                    })
                })
                .catch((error) => {
